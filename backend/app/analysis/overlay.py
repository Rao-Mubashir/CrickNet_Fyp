...
def main():
    ...
    import os
    if len(sys.argv) < 5:
        print("Usage: python overlay.py <video_path> <output_path> <original_coords> <predicted_coords>")
        sys.exit(1)
        
    video_path = sys.argv[1]
    output_path = sys.argv[2]
    original_coords = read_coordinates(sys.argv[3])
    predicted_coords = read_coordinates(sys.argv[4])
    ...
    if snapshot_frame is not None:
        cv2.imwrite(str(Path("output") / "snapshot_with_spin_angle.png"), snapshot_frame)
