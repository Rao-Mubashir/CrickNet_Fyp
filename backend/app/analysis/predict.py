...
def main():
    import sys
    if len(sys.argv) < 3:
        print("Usage: python predict.py <input_coords.txt> <output_coords.txt>")
        sys.exit(1)
    ...
    if len(coords) < 4:
        print(f"Not enough coordinates to detect a bounce: found {len(coords)} points")
        return
    ...
    if bounce_frame is None:
        print("Bounce not detected. Check whether coord.py is writing detections and whether the ball crosses a visible peak in Y.")
        return
